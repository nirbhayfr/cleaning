...
import { useOrderCompleted } from "../../hooks/useorders";
...
export default function MyOrders() {
  ...
  const { data: orders, isLoading, error } = useFetchUserOrders(userId);
    ...
    <>
      {orders.map((order: Order) => (
        <div key={order._id} className="order-card">
          <span className="order-category">{order.products[0]?.category}</span>
